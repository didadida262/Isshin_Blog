---
title: 现代单页应用（SPA）中微信授权登录的高可用架构实现
date: 2026-04-16 16:05:45
category:
---
...
在子窗口完成授权后，如何通知父窗口“登录成功”并同步状态？本项目采用了**多层级通信机制**。

1.  **PostMessage（首选）** ：通过 `window.opener.postMessage` 向父窗口发送包含 `token` 和重定向路径的消息。
1.  **BroadcastChannel（备选）** ：若 `opener` 引用丢失，利用 `BroadcastChannel` 建立跨页面通信隧道。
1.  **Storage Event（保底）** ：若上述 API 均失效，通过修改 `localStorage` 触发 `storage` 事件，实现父子窗口的被动同步。
